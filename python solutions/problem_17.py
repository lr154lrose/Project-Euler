...
def add_up_letters(i, n):
    ...
    for number in range(i, n+1):
        if(len(str(number)) == 1): sum_of_letters += one_digit[number]
        if(len(str(number)) == 2): 
            if(number in teens): sum_of_letters += teens[number]
            else:
                sum_of_letters += one_digit[number%10]
                sum_of_letters += tens[number - number%10]

        if(len(str(number)) == 3):
            if(number == 100): sum_of_letters += 10
            else: sum_of_letters += one_digit[number//100] + _hundred
            if(number%100 in teens):
                sum_of_letters += teens[number%100]
                sum_of_letters += _and
            else:
                sum_of_letters += one_digit[number%10]
                if(number - number%10 - number//100*100 == 0 and number%10 == 0): pass
                else: 
                    sum_of_letters += _and
                    sum_of_letters += tens[number - number%10 - number//100*100]

        if(len(str(number)) == 4): sum_of_letters += len("onethousand")
